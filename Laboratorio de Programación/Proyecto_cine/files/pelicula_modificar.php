        ...
        if(!$id){
            ...
            if($accion=="actualizar"){
                ...
                if (empty($_FILES['imagen']['name'])) {
                    ...
                    unlink('../img/peliculas/'.$pelicula->getImagen());
                ...
                if($pelicula->actualizar($conexion)){
                    ...
                    echo '<button type="button" class="close" data-dismiss="alert">&times;</button>';
                    echo '<strong>Se actualizaron los datos de la película en la base de datos.</strong>';
                    echo '</div>';
                    echo '</div>';
                }
                else{
...
?>
            ...
            <div class="container-fluid col-xs-10 col-sm-6 col-md-6 col-lg-6">
                <div class="panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title">Datos de la película</h3>
                    </div>
                    <div class="panel-body">
                        <form autocomplete="off" method="post" enctype='multipart/form-data' action="files/pelicula_modificar.php" onSubmit="return validarActualizacionPelicula()" class="form">
                            <div class="form-group">
                                <div class="input-group">
                                    <span class="input-group-addon">ID</span>
                                    <input id="id" name="id" type="text" value="<?php echo $pelicula->getId();?>" tabindex="-1" class="form-control" readonly/>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <select id="genero" name="genero" tabindex="9" class="form-control">
                                    <option value="0" disabled selected hidden>Genero</option>
                                    <?php
                                        if($listado != null){
                            ...
                            <div class="text-center">
                                <button id="actualizar" name="actualizar" type="submit" class="btn btn-primary" tabindex="12">Actualizar</button>
                                <input type="hidden" name="accion" value="actualizar"/>
                                <button id="limpiar" name="limpiar" type="reset" class="btn btn-default" tabindex="13" onclick="eliminarImagen(); <?php echo 'cargarImagen(\''.$pelicula->getImagen().'\')';?>">Limpiar</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div class="row row-centrada">
                <a href="files/pelicula_busqueda.php" id="btn-inicio"><img src="img/botones/regresar-negro.svg" class="link-img"></a>
        </div>
        <script>
            $(document).ready(function(){
                $("#btn-regresar").attr("href", document.referrer);
            });
        </script>
        <script type="text/javascript">if(<?php echo '\''.$pelicula->getImagen().'\'';?>!=""){<?php echo 'cargarImagen(\''.$pelicula->getImagen().'\')';?>}else{cargarImagen("noImage.png");}</script>
    </body>
</html>